...
public class RedisUtil {
    ...
    public String getInfo(String param) {
        ...
        try {
            ...
            if(param==null){
                ...
                redisInfo = conn.info(p[0]);
                if(p.length>1){
                    Properties tmp = new Properties();
                    for (int i = 1; i < p.length; i++) {
                        String k = p[i];
                        Object v = redisInfo.get(k);
                        tmp.setProperty(  k,  v==null?"":v.toString() );
                    }
                    redisInfo = tmp;
                }
            }
        ...
        return JSON.toJSONString(redisInfo);
    }
    ...
    public String getHits() {
        ...
        Properties tmp = new Properties();
        tmp.setProperty(  "命中次数",  hits  );
        tmp.setProperty(  "未命中次数",  misses  );
        tmp.setProperty(  "命中率",  getPercentFormat(hitsRate, 2,2)  );

        return JSON.toJSONString(tmp);
    }

    public void publish(byte[] topic, byte[] msg) {
        redisTemplate.getConnectionFactory().getConnection().publish(topic,msg);
    }
